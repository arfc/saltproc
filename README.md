...
`saltproc/saltproc.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `saltproc.py` in directory called
`saltproc`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import saltproc as sb` in an
interactive Python session, the classes and functions defined inside of the
`saltproc.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the saltproc module, the main classes defined at the bottom of
...
Either way, you can create a `saltproc/data` folder in which you can
...
(typically, something like our `saltproc/tests/test_saltproc.py`). Within each
...
have one such test in `saltproc/tests/test_saltproc.py` called
...
will see a message such as:
    ...
    saltproc/tests/test_saltproc.py .F...
    ...
    saltproc/tests/test_saltproc.py:49: AssertionError
...
To document `saltproc` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/saltproc/blob/master/doc/_static/RTD-advanced-conf.png)

 http://saltproc.readthedocs.org/en/latest/
...
`saltproc` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `saltproc`, we use the
...
[here](https://github.com/uwescience/saltproc/blob/master/scripts/Figure1.ipynb).
...
Under "old repository clone URL" enter: https://github.com/uwescience/saltproc
...
`saltproc/saltproc.py` to be called `smallish/smallish.py`

	git mv saltproc smallish
	git mv smallish/saltproc.py smallish/smallish.py
	git mv smallish/tests/test_saltproc.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `saltproc` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `saltproc` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `saltproc` in them. Type
the following to see where all these files are:

	git grep saltproc

You can replace `saltproc` for `smallish` quickly with:

	git grep -l 'saltproc' | xargs sed -i 's/saltproc/smallish/g'
...
Edit all the mentions of `saltproc` in the `saltproc/__init__.py` file, and in
the `saltproc/version.py` file as well.
