...
class saltproc_one_region:
    """ Class saltproc runs SERPENT and manipulates its input and output files
        ...
        HDF5 database. This class is for one region flows.
    """
    ...
    def __init__(self, steps, cores, nodes, bw, exec_path, restart=False,
        ...
        fuel_mat_name: string
            name of fuel material in SERPENT input
        salt_vol: float
            volume of salt in cm^3
        """
    ...
    def init_db(self):
        ...
        self.waste_tank_db = self.f.create_dataset('waste tank composition',
                                                   shape, maxshape=maxshape,
                                                   chunks=True)
        self.refill_tank_db = self.f.create_dataset('salt refill tank composition',
                                                    shape, maxshape=maxshape,
                                                    chunks=True)
        # initial composition
        ...
        # initial composition
        self.dep_dict = self.read_dep()
        self.comp_before_db[0, :] = self.dep_dict[self.fuel_mat_name] * self.salt_vol
        self.comp_after_db[0, :] = self.dep_dict[self.fuel_mat_name] * self.salt_vol
    ...
    def reopen_db(self, restart):
        ...
        self.comp_before_db = self.f['salt composition before reproc']
        
        self.comp_after_db = self.f['salt composition after reproc']
        self.waste_tank_db = self.f['waste tank composition']
        self.refill_tank_db = self.f['salt refill tank composition']
        ...
        if restart:
            ...
            self.comp_before_db.resize(shape)
            self.comp_after_db.resize(shape)
            ...
            self.core = {}
            self.core[self.fuel_mat_name] = self.comp_after_db[self.current_step - 1]
            self.write_mat_file()
    ...
    def separate_fuel(self):
        ...
        self.core[self.fuel_mat_name] = self.core[self.fuel_mat_name] * self.salt_vol
        ...
        self.waste_tank_db[self.current_step, :] += self.remove_iso(volatile_gases, 1, self.fuel_mat_name)

        # remove noble metals
        noble_metals = self.find_iso_indx(['Se', 'Nb', 'Mo', 'Tc', 'Ru', 'Rh', 'Pd',
                                           'Ag', 'Sb', 'Te', 'Zr', 'Cd', 'In', 'Sn'])
        self.waste_tank_db += self.remove_iso(noble_metals, 1, self.fuel_mat_name)

        # remove rare earths
        rare_earths = self.find_iso_indx(['Y', 'La', 'Ce', 'Pr', 'Nd', 'Pm', 'Sm',
                                          'Gd', 'Eu', 'Dy', 'Ho', 'Er', 'Tb', 'Ga',
                                          'Ge', 'As', 'Zn'])
        self.waste_tank_db += self.remove_iso(rare_earths, 1, self.fuel_mat_name)
        """
        # remove every 10 years
        if self.current_step % 1216 == 0:
           discard = self.find_iso_indx(['Cs', 'Ba', 'Rb', 'Sr', 'Li', 'Be',
                                         'Cl', 'Na' 'Th'])
           self.waste_tank_db += self.remove_iso(discard_id, 1, self.fuel_mat_name)
        """
    ...
    def refuel(self):
        ...
        print('REMOVED A TOTAL OF %f KG OF WASTE FROM THE FUEL SALT' %fill_qty)
    ...
    def reactivity_control(self):
        ...
        pu_loss = self.comp_before_db[self.current_step, pu] - self.comp_after_db[self.current_step-1, pu]
        pu_loss = sum(pu_loss)
        ...
        if self.eoc_keff[0] > 1.05:
            print('KEFF IS TOO HIGH: NOT PUTTING ANY MORE PU IN FUEL SALT\n')
            qty = 0
    ...
    def record_db(self):
        ...
        self.comp_after_db[self.current_step, :] = self.core[self.fuel_mat_name]
        self.f.close()
    ...
    def refill(self, refill_iso, delta, region):
        ...
        for iso in refill_iso:
            ...
            if region == self.fuel_mat_name:
                self.refill_tank_db[self.current_step, iso] -= delta
            else:
                raise ValueError('Wrong fuel material name')
