...
class saltproc_two_region:
    ...
    def refuel(self):
        ...
        self.fill_qty = sum(self.waste_tank_db[self.current_step, :])
        print('REMOVED A TOTAL OF %f GRAMS OF WASTE FROM THE DRIVER' %self.fill_qty)

        #!
        pu_indx = self.find_iso_indx(['Pu'])
        #!
        #! DELETED FOR ANDREI
        ...
        if sum(pu_to_add) >= self.fill_qty:
            raise ValueError('\n\nThe fissile add back fraction is too high. '
                             'It causes the driver to increase in mass!!!!\n\n')

        self.fissile_tank_db[self.current_step, :] -= pu_to_add
        for indx, i in enumerate(fissile_tank_db[self.current_step, :]):
            if i < 0:
                print('%s is negative.. WHY?' %self.isoname[i])
        #!
        print('Before adding to core')
        print(self.isoname[pu_indx])
        print(self.core[self.driver_mat_name][pu_indx])
        self.core[self.driver_mat_name] += pu_to_add
        print('After adding to core')
        print(self.core[self.driver_mat_name][pu_indx])

        # fill the rest of the driver with depleted uranium
        self.fill_qty -= sum(pu_to_add)
        u238_fill = self.fill_qty * (1 - tails_enrich)
        u235_fill = self.fill_qty * tails_enrich
    ...
    def reactivity_control(self):
        ...
        if self.eoc_keff[0] > 1.03:
            print('KEFF IS TOO HIGH: NOT PUTTING ANY MORE PU IN DRIVER\n')
            qty = 0
        elif self.eoc_keff[0] <= 1.03 and self.eoc_keff[0] > 1.01:
            print('KEFF IS IN A GOOD SPOT: PUTTING THE AMOUNT LOST FROM PREV DEPLETION\n')
            qty = pu_loss
        elif self.eoc_keff[0] <= 1.01:
            print('KEFF IS LOW: PUTTING IN 1.2 TIMES PU THAN PREIVOUS STEP:')
            qty = self.prev_qty * 1.2
        if qty > pu_avail:
            print('NOT ENOUGH PU AVAILABLE: PUTTING THE MAXIMUM AMOUNT AVAILABLE')
            qty = pu_avail
        elif qty < self.fill_qty:
            print('Cannot put in more than what has been taken out of driver. \n'
                  'It will cause increase in mass. Putting in the maximum amount')
            qty = self.fill_qty
        if qty != 0:
            self.prev_qty = qty
        return qty
